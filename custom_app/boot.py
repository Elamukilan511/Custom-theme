	...
	for page in all_pages:
		try:
			...
			if has_access or workspace.is_permitted():
				...
				ws_sh = frappe.get_all("Workspace Shortcut", fields=['name','link_to','type'], filters={'parent':page.name,'type':['!=','URL']}, order_by='idx desc', ignore_permissions=True )
				page["submenu"] = '' if not ws_sh else f'<ul class="sub-menu">\n ' + '\n'.join(map(lambda item: f'    <li><a href="/app/{item["link_to"].lower().replace(" ", "-")}">{item["link_to"]}</a></li>', ws_sh)) + '</ul>'
